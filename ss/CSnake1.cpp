...
void CSnake::Game_Main()
{
	...
	} while (life >= 1); //목숨이 1개 이상일 때까지(=0개 이하면 GameOver)
	Game_Over();
...
{
	if ((apple+1)%3 == 0 &&flag==1)set_item();
...
void CSnake::set_item() {
	...
	for (i = 1; i <= snake; i++)
	{
		if ((randomA == snakeXLocation[i]) && (randomB == snakeYLocation[i])) set_item();
